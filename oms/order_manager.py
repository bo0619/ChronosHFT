...
class OrderManager:
    ...
    def on_order_submitted(self, event):
        ...
        if data.status in {
            ...
            with self.lock:
                self.monitored_orders.pop(data.order_id, None)
            return
        submitted_monotonic = float(
            getattr(data, "monotonic_timestamp", 0.0) or data.timestamp
        )
        with self.lock:
            if data.order_id in self.monitored_orders:
                return
    ...
    def track_prepared_order(self, order):
        """Track a durably prepared order before transport can emit updates."""
